...
[num_reefs, lon, lat, reef_area, omega] = match_reefs_to_areas(lg, lt, psurv_d02_1122_P7);
...
params.p_1_f = ...
    (0.129/2500)*reef_area;     % effect of COTS on fast-growing coral
...
params.p_2_f = ...
    (10/2500)*reef_area;        % effect of COTS on fast-growing coral
...
params.omega_c = omega;         % coral
params.omega_s = omega * 0.3;   % starfish
...
% CORAL
% Percentage of fast-growing coral = 30% everywhere
initial_coral = 0.5;
...
%% CALCULATE: Histogram calculations
...
ylabel('Total coral cover', 'Interpreter', 'Latex', ...
    'Fontsize', axis_FS)
title('Total coral cover on GBR (sqkm)', ...
...
set(gca, 'FontSize', ticks_FS);
xlabel('Time (years)', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Total coral cover increase (sqkm)', 'Interpreter', 'Latex', ...
    'Fontsize', axis_FS)
...
set(gca, 'FontSize', ticks_FS);
xlabel('Time (years)', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Total coral cover (sqkm)', 'Interpreter', 'Latex', ...
    'Fontsize', axis_FS)
...
for i = 1:num_reefs
    if starfish_final_s0_sorted(i) >= 500
        starfish_final_reefs(index_500) = starfish_final_s0_sorted(i);
...
title({['Coral cover increase after ', num2str(t_end), ' years with 100\% effort'], ...
    'at 168 reefs compared to no control'}, ...
    'Interpreter', 'Latex', 'Fontsize', title_FS)
% Add colorbar
c = colorbar;
cmap = colormap(viridis);
