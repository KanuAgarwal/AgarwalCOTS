...
% Reefs with more than 500 starfish (no control) --------------------------
...
% % Reefs with more than 500 starfish (control) -----------------------------
% % Get adult starfish at final timestep
% starfish_final_s1 = N_y_2_s1(:, end);
% starfish_final_s2 = N_y_2_s2(:, end);
% starfish_final_s3 = N_y_2_s3(:, end);
% starfish_final_s4 = N_y_2_s4(:, end);
% 
% % Sort final starfish by latitude of reefs 
% [lat_sorted, sort_index] = sort(lat);
% starfish_s1_final_sorted = starfish_final_s1(sort_index);
% starfish_s2_final_sorted = starfish_final_s2(sort_index);
% starfish_s3_final_sorted = starfish_final_s3(sort_index);
% starfish_s4_final_sorted = starfish_final_s4(sort_index);
% 
% % Initialise indexes for new arrays
% index_500_s1 = 1;
% index_500_s2 = 1;
...
% for i = 1:num_reefs
%     % Compare scenario 1
%     if starfish_s1_final_sorted(i) >= 500
%         starfish_final_reefs_s1(index_500_s1) = starfish_s1_final_sorted(i);
%         lat_starfish_values_s1(index_500_s1) = lat_sorted(i);
%         index_500_s1 = index_500_s1 + 1;
%     end
%     % Compare scenario 2
%     if starfish_s2_final_sorted(i) >= 500
%         starfish_final_reefs_s2(index_500_s2) = starfish_s2_final_sorted(i);
%         lat_starfish_values_s2(index_500_s2) = lat_sorted(i);
%         index_500_s2 = index_500_s2 + 1;
%     end
%     % Compare scenario 3
%     if starfish_s3_final_sorted(i) >= 500
%         starfish_final_reefs_s3(index_500_s3) = starfish_s3_final_sorted(i);
%         lat_starfish_values_s3(index_500_s3) = lat_sorted(i);
%         index_500_s3 = index_500_s3 + 1;
%     end
%     % Compare scenario 4
%     if starfish_s4_final_sorted(i) >= 500
%         starfish_final_reefs_s4(index_500_s4) = starfish_s4_final_sorted(i);
%         lat_starfish_values_s4(index_500_s4) = lat_sorted(i);
%         index_500_s4 = index_500_s4 + 1;
%     end
% end
% 
% % Number of reefs with 500 starfish or more
% index_500_s1
% index_500_s2
% index_500_s3
% index_500_s4
...
title(['Adult COTS population ', num2str(t_end), ' years after outbreak'], ...
    'Interpreter', 'Latex', 'Fontsize', title_FS)
c2.Label.String = 'No. of adult (age 2+) COTS';
c2.Label.Interpreter = 'Latex';
c2.Label.FontSize = 14;
...
ylim([-25, -10])
xlabel('No. of reefs with $\geq500$ adult (age 2+) COTS', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Latitude', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
title({'Latitudinal spread of reefs with at least 500 ', ...
    ['adult COTS after ', num2str(t_end), ' years']}, ...
    'Interpreter', 'Latex', 'Fontsize', title_FS)

% % Histogram for all four scenarios ----------------------------------------
% p3 = subplot(1, 3, 3); hold on, box on
% 
% % Plot histogram first and get info
% h1_s = histogram(lat_starfish_values_s1, 40, 'Orientation', 'horizontal');
% h2_s = histogram(lat_starfish_values_s2, 40, 'Orientation', 'horizontal');
% h3_s = histogram(lat_starfish_values_s3, 40, 'Orientation', 'horizontal');
% h4_s = histogram(lat_starfish_values_s4, 40, 'Orientation', 'horizontal');
% h1_s.BinWidth = 0.5;
% h2_s.BinWidth = 0.5;
% h3_s.BinWidth = 0.5;
% h4_s.BinWidth = 0.5;
...
% s1_reefs_s = h1_s.Values;
% s2_reefs_s = h2_s.Values;
% s3_reefs_s = h3_s.Values;
% s4_reefs_s = h4_s.Values;
% 
% % y axis values
% lat_values_s1_s = h1_s.BinEdges + (h1_s.BinWidth/2);
% lat_values_s1_s = lat_values_s1_s(2:end);
% lat_values_s2_s = h2_s.BinEdges + (h2_s.BinWidth/2);
% lat_values_s2_s = lat_values_s2_s(2:end);
% lat_values_s3_s = h3_s.BinEdges + (h3_s.BinWidth/2);
% lat_values_s3_s = lat_values_s3_s(2:end);
% lat_values_s4_s = h4_s.BinEdges + (h4_s.BinWidth/2);
% lat_values_s4_s = lat_values_s4_s(2:end);
% 
% % Create arrays and delete rows with zeros
% s1_reefs_plot_s = [s1_reefs_s; lat_values_s1_s]';
% s1_reefs_plot_s = s1_reefs_plot_s(all(s1_reefs_plot_s, 2), :);
% s2_reefs_plot_s = [s2_reefs_s; lat_values_s2_s]';
% s2_reefs_plot_s = s2_reefs_plot_s(all(s2_reefs_plot_s, 2), :);
% s3_reefs_plot_s = [s3_reefs_s; lat_values_s3_s]';
% s3_reefs_plot_s = s3_reefs_plot_s(all(s3_reefs_plot_s, 2), :);
% s4_reefs_plot_s = [s4_reefs_s; lat_values_s4_s]';
% s4_reefs_plot_s = s4_reefs_plot_s(all(s4_reefs_plot_s, 2), :);
% 
% % Clear this subplot
% cla(p3)
% 
% % Now make the scatter plot
% s1_s = scatter(s1_reefs_plot_s(:, 1), s1_reefs_plot_s(:, 2), 'o');
% s1_s.MarkerFaceColor = colour_scheme(1, :);
% s1_s.MarkerEdgeColor = colour_scheme(1, :);
% s1_s.SizeData = 70;
% s1_s.LineWidth = 2;
% 
% s2_s = scatter(s2_reefs_plot_s(:, 1), s2_reefs_plot_s(:, 2), 'x');
% s2_s.MarkerFaceColor = colour_scheme(2, :);
% s2_s.MarkerEdgeColor = colour_scheme(2, :);
% s2_s.SizeData = 90;
% s2_s.LineWidth = 2;
% 
% s3_s = scatter(s3_reefs_plot_s(:, 1), s3_reefs_plot_s(:, 2), '+');
% s3_s.MarkerFaceColor = colour_scheme(3, :);
% s3_s.MarkerEdgeColor = colour_scheme(3, :);
% s3_s.SizeData = 90;
% s3_s.LineWidth = 2;
% 
% s4_s = scatter(s4_reefs_plot_s(:, 1), s4_reefs_plot_s(:, 2), 'o');
% s4_s.MarkerFaceColor = colour_scheme(4, :);
% s4_s.MarkerEdgeColor = colour_scheme(4, :);
% s4_s.SizeData = 70;
% s4_s.LineWidth = 2;
% 
% % Set the axis, labels etc. 
% set(gca, 'FontSize', ticks_FS)
% % p3.Position = [0.6 0.08 0.35 0.84];
% ylim([-25, -10])
% % xlim([0 60])
% xlabel('No. of reefs with $\geq500$ adult (age 2+) COTS', ...
