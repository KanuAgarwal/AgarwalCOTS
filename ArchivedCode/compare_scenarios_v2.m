...
params.omega_s = psurv_d02_1122_P7 * 0.3;       % starfish
...
initial_coral = 0.8;
initial_state.C_0_f = initial_coral * params.K_f * ones(num_reefs, 1);

% STARFISH
% Initialise array for number of COTS
initial_state.N_0_2 = zeros(num_reefs, 1);
initial_state.N_0_1 = zeros(num_reefs, 1);
initial_state.N_0_0 = zeros(num_reefs, 1);

% Put starfish on all reefs within initiation box -------------------------
...
initial_state.N_0_1 = initial_state.N_0_2 * exp(params.M_cots ...
    * (1 - params.p_tilde*(initial_coral/(1+initial_coral)))) ;
initial_state.N_0_0 = initial_state.N_0_2 * exp(2*params.M_cots ...
    * (1 - params.p_tilde*(initial_coral/(1+initial_coral))));


% % Create an index to keep track of the number of reefs found
% box_index = 0;
% 
% % Find reefs within the initiation box, and create an array for it
% for i = 1:num_reefs
%     if (lat(i) > -17 && lat(i) < -14.75) && (lon(i) > 145 && lon(i) < 147)
%         box_index = box_index + 1;
%         initiation_box_reefs(box_index) = i;
%     end
% end
%
% % Select a random 30 reefs from the initiation box 
% rand_box_reefs = randsample(initiation_box_reefs, 30);
% 
% % Calculate initial age 2+, age 1 and age 0 starfish
% initial_cots_age_2 = 100;
% initial_cots_age_1 = initial_cots_age_2 * exp(params.M_cots ...
%     * (1 - params.p_tilde*(initial_coral/(1+initial_coral))));
% initial_cots_age_0 = initial_cots_age_1 * exp(2*params.M_cots ...
%     * (1 - params.p_tilde*(initial_coral/(1+initial_coral))));
% 
% % Loop over every random reef
% for i = 1:3
%     % Get the reef index
%     current_reef = rand_box_reefs(i);
%     if i == 1
%         % For the first 10 reefs, initialise all age classes
%         initial_state.N_0_2(current_reef) = initial_cots_age_2;
%         initial_state.N_0_1(current_reef) = initial_cots_age_1;
%         initial_state.N_0_0(current_reef) = initial_cots_age_0;
%     elseif i == 2
%         % For the next 10 reefs, initialise only age 1 and 0 classes
%         initial_state.N_0_1(current_reef) = initial_cots_age_1 ;
%         initial_state.N_0_0(current_reef) = initial_cots_age_0;
%     elseif i == 3 
%         % For the last 10 reefs, initialise only age 0 classes
%         initial_state.N_0_0(current_reef) = initial_cots_age_0;
%     end
% end
...
% % Create an index to keep track of the number of reefs
% num_reefs_control_s1 = 0;
% 
% % Find reefs within the initiation box, and store indexes in an array
% for i = 1:num_reefs
%     if (lat(i) > -17 && lat(i) < -14.75) && (lon(i) > 145 && lon(i) < 147)
%         num_reefs_control_s1 = num_reefs_control_s1 + 1;
%         initiation_box_reefs(num_reefs_control_s1) = i;
%     end
% end
% 
% % Initialise array for control effort over time
% control_effort_s1 = zeros(num_reefs, t_end);
% 
% % Cull all starfish only in the initiation box but over 2 years 
% for k = 1:t_end
%     % Randomly select half of the reefs every 2 years 
%     if mod(k, 2) == 1
%         % Randomise reefs in initiation box
%         rand_reefs = randsample(initiation_box_reefs, num_reefs_control_s1);
%         % Select first half of the reefs 
%         rand_reefs_subset = rand_reefs(1:num_reefs_control_s1/2);
%     elseif mod(k, 2) == 0
%         % Select the other half of the reefs 
%         rand_reefs_subset = rand_reefs((num_reefs_control_s1/2+1):end);
%     end
%     
%     % Cull starfish at half the reefs this year 
%     for i = 1:length(rand_reefs_subset)
%         current_reef = rand_reefs_subset(i);
%         control_effort_s1(current_reef, k) = 1;
%     end
% end
% 
% % Count number of reefs controlled in control scenario 1
% num_reefs_control_s1
% budget_s1 = sum(control_effort_s1, 'all')
...
% % Create an index to keep track of the number of reefs
% num_reefs_control_s2 = 0;
% 
% % Find reefs we want to control, and store indexes in an array
% for i = 1:num_reefs
%     if (lat(i) > -18.45 && lat(i) < -14) && (lon(i) > 143 && lon(i) < 147.75)
%         num_reefs_control_s2 = num_reefs_control_s2 + 1;
%         scenario_2_reefs(num_reefs_control_s2) = i;
%     end
% end
% 
% % Initialise array for control effort over time
% control_effort_s2 = zeros(num_reefs, t_end);
% 
% % Cull all starfish at twice as many reefs but over 2 years 
% for k = 1:t_end
%     % Randomly select half of the reefs every 2 years 
%     if mod(k, 2) == 1
%         % Randomise reefs
%         rand_reefs = randsample(scenario_2_reefs, num_reefs_control_s2);
%         % Select first half of the reefs 
%         rand_reefs_subset = rand_reefs(1:num_reefs_control_s2/2);
%     elseif mod(k, 2) == 0
%         % Select the other half of the reefs 
%         rand_reefs_subset = rand_reefs((num_reefs_control_s2/2+1):end);
%     end
%     
%     % Cull starfish at half the reefs each year 
%     for i = 1:length(rand_reefs_subset)
%         current_reef = rand_reefs_subset(i);
%         control_effort_s2(current_reef, k) = 0.5;
%     end
% end
% 
% % Count number of reefs controlled in control scenario 2
% num_reefs_control_s2
% budget_s2 = sum(control_effort_s2, 'all')
...
% % Create an index to keep track of the number of reefs
% num_reefs_control_s3 = 0;
%
% % Find reefs we want to control, and store indexes in an array
% for i = 1:num_reefs
%     if (lat(i) > -20 && lat(i) < -12.69) && (lon(i) > 142 && lon(i) < 152)
%         num_reefs_control_s3 = num_reefs_control_s3 + 1;
%         scenario_3_reefs(num_reefs_control_s3) = i;
%     end
% end
%
% % Initialise array for control effort over time
% control_effort_s3 = zeros(num_reefs, t_end);
% 
% % Cull all starfish on four times as many reefs but over 2 years 
% for k = 1:t_end
%     % Randomly select half of the reefs every 2 years 
%     if mod(k, 2) == 1
%         % Randomise reefs 
%         rand_reefs = randsample(scenario_3_reefs, num_reefs_control_s3);
%         % Select first half of the reefs 
%         rand_reefs_subset = rand_reefs(1:num_reefs_control_s3/2);
%     elseif mod(k, 2) == 0
%         % Select the other half of the reefs 
%         rand_reefs_subset = rand_reefs((num_reefs_control_s3/2+1):end);
%     end
%     
%     % Cull starfish at half the reefs each year 
%     for i = 1:length(rand_reefs_subset)
%         current_reef = rand_reefs_subset(i);
%         control_effort_s3(current_reef, k) = 0.25;
%     end
% end
% 
% % Count number of reefs controlled in control scenario 3
% num_reefs_control_s3
% budget_s3 = sum(control_effort_s3, 'all')
...
% % Calculate percentage of starfish to cull
% effort_per_2_years = 2 * budget_s1 / t_end;
% effort_per_reef = effort_per_2_years / num_reefs
% 
% % Create array for scenario 4 reefs
% scenario_4_reefs = 1:1:num_reefs;
% num_reefs_control_s4 = num_reefs;
% 
% % Initialise array for control effort over time
% control_effort_s4 = zeros(num_reefs, t_end);
% 
% % Cull all starfish on entire GBR but over 2 years 
% for k = 1:t_end
%     % Randomly select half of the reefs every 2 years 
%     if mod(k, 2) == 1
%         % Randomise reefs 
%         rand_reefs = randsample(scenario_4_reefs, num_reefs_control_s4);
%         % Select first half of the reefs 
%         rand_reefs_subset = rand_reefs(1:ceil(num_reefs_control_s4/2));
%     elseif mod(k, 2) == 0
%         % Select the other half of the reefs 
%         rand_reefs_subset = rand_reefs((ceil(num_reefs_control_s4/2)+1):end);
%     end
%     
%     % Cull starfish at half the reefs each year 
%     for i = 1:length(rand_reefs_subset)
%         current_reef = rand_reefs_subset(i);
%         control_effort_s4(current_reef, k) = effort_per_reef;
%     end
% end
% 
% % Count number of reefs controlled in control scenario 4
% num_reefs_control_s4
% budget_s4 = sum(control_effort_s4, 'all')
...
for i = 1:size(coral_end, 2)
    ...
    coral_compare_all(i, 1) = sum(coral_end(:, i) < 0.3);
    
    % Count the number of reefs with between 1% and 5% coral
    coral_compare_all(i, 2) = sum(coral_end(:, i) >= 0.3 & coral_end(:, i) < 0.5);
    
    % Count the number of reefs with between 5% and 30% coral
    coral_compare_all(i, 3) = sum(coral_end(:, i) >= 0.5 & coral_end(:, i) < 0.75);
    
    % Count the number of reefs with more than 30%
    coral_compare_all(i, 4) = sum(coral_end(:, i) >= 0.75);
end
...
%% COMPARE: Compare best scenario to worst scenario

% % Comparing density of reefs with coral increase --------------------------
% num_plot = 435;
...
starfish_final_s0_sorted = starfish_final_s0(sort_index);

% Initialise indexes for new arrays
index_1000 = 1;

% Select data with reefs that have at least 1000 starfish
for i = 1:num_reefs
    if starfish_final_s0_sorted(i) >= 1
        starfish_final_reefs(index_1000) = starfish_final_s0_sorted(i);
        lat_starfish_values(index_1000) = lat_sorted(i);
        index_1000 = index_1000 + 1;
    end
end
...
ylim([-26, -8])
xlabel('No. of reefs with $\geq1000$ adult starfish', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Latitude', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
title({'Latitudinal spread of reefs with at least 1000 ', ...
...
% pt = patch(Outline(:, 1), Outline(:, 2), [1 1 1], 'FaceColor', [0.8 0.8 0.8]);
% % Plot reef locations by color depending on initial cots numbers
% scatter(lon, lat, 12, C_y_f_s3(:, end), 'filled')
% % Focus the figure on GBR and QLD
% xlim([140, 155])
% ylim([-26, -8])
% % Add labels
...
legend('$x<30$\% coral cover', '$30\%\leq x<50\%$ coral cover', ...
    '$50\%\leq x<75\%$ coral cover', '$x\geq 75\%$ coral cover', ...
...
coral_best_compare = (C_y_f_s3(:, end) - C_y_f_s0(:, end)) * 100;
...
% Subplot of coral cover increase -----------------------------------------
figure(25), clf
...
xlabel('Longitude', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Latitude', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
title({['Coral cover increase after ', num2str(t_end), ' years with 25\% effort'], ...
    'at 672 reefs compared to no control'}, ...
    'Interpreter', 'Latex', 'Fontsize', title_FS)
% title({['Coral cover increase after ', num2str(t_end), ' years with 7.72\% effort'], ...
%     'at 2175 reefs compared to no control'}, ...
%     'Interpreter', 'Latex', 'Fontsize', title_FS)
% Add colorbar
c = colorbar;
cmap = colormap(viridis);
