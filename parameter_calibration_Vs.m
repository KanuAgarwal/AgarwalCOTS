...
% CHANGE LARVAL SURVIVAL PARAMATERS ---------------------------------------
% Connectivity matrices from Bode et al. (2012)
V_f = 1;                      % coral larval survival rate
V_s = 0.25;                      % starfish larval survival rate
params.omega_c = V_f*omega;     % coral larval dispersal
params.omega_s = V_s*omega;     % starfish larval dispersal
...
initial_Vs_vals = categorical({'0.3', '0.5', '0.7', '0.9', '1'});
initial_Vs_vals = reordercats(initial_Vs_vals, {'0.3', '0.5', '0.7', '0.9', '1'});
...
scatter(initial_Vs_vals, scenario_0_coral, 100, 'black', 'filled')
scatter(initial_Vs_vals, scenario_1_coral, 100, colour_scheme(1, :), 'filled')
scatter(initial_Vs_vals, scenario_2_coral, 100, colour_scheme(2, :), 'filled')
scatter(initial_Vs_vals, scenario_3_coral, 100, colour_scheme(3, :), 'filled')
scatter(initial_Vs_vals, scenario_4_coral, 100, colour_scheme(4, :), 'filled')
set(gca, 'FontSize', ticks_FS);
ylim([3200 3600])
set(gca, 'YTick', 3200:100:3600);
xlabel('$V_s$', 'Interpreter', 'Latex', 'Fontsize', axis_FS)
ylabel('Total coral cover ($km^2$)', 'Interpreter', 'Latex', ...
    'Fontsize', axis_FS)
title('\qquad\qquad\qquad\qquad\qquad Total coral cover on GBR after 100 years for varying $V_s$', ...
...
saveas(gcf, 'Plots/03_paper/V_s_parameter_calibration.png')